...
export const filterResponse = (dgResponseData) => {
  ...
  return finalData.filter((d, i) => moment(d.Date).isBefore(moment(new Date()))).filter((d) => moment(d.Date).dayOfYear() > moment(new Date()).dayOfYear() - 30);
};
