...
export function makeEngagementChart(svgId, data = {percent: 35}) {
  ...
  var radius = Math.min(width, height) / 2, pie = dPie().sort(null), format = dFormat(".0%");
  ...
  var text = CHART_G.append("text").attr("text-anchor", "middle").attr("dy", ".3em").attr("fill", "white").attr("font-family", "var(--zss-metric-font)").attr("font-size", height * 0.15 + "px");
  var progress = 0;
