import { select, max, scaleLinear, extent, axisBottom, axisRight, scaleBand, interpolateWarm, scaleQuantize, } from 'd3';
let SVG;
let CHART_G;
let x, y;
export const makeSiteEnergyUsage = (svgId, responseData) => {
    ...
    CHART_G.append('rect')
        ...
        .attr('ry', 7);
    const renderedData = responseData
        .reduce((arr, i) => {
        var _a;
        let falseArray = [];
        (_a = i.children) === null || _a === void 0 ? void 0 : _a.forEach((child) => falseArray.push(child));
    ...
    console.log({ data });
    const yMax = Math.ceil((_a = max(data, (d) => Number(d.KwH))) !== null && _a !== void 0 ? _a : 0);
    const padY = yMax * 0.4;
    ...
    const xAxis = (g, x) => {
        g.attr('transform', `translate(${PAD + margin.left + 5}, ${height + margin.top})`).call(axisBottom(x)
            .scale(x)
            .tickSize(5)
            .ticks(10)
            .tickFormat((d) => d.title));
    ...
    const yAxis = (g, y) => {
        g.attr('transform', `translate(${-margin.left}, ${height + margin.top})`).call(axisRight(y)
            .scale(y)
            .tickSize(-width + 70)
            .ticks(5));
    ...
    const colorDomain = scaleQuantize()
        .domain(extent(data.flatMap((d) => d.KwH)))
        .range([
        'var(--zss-nominal)',
        'var(--zss-nominal)',
        'var(--zss-nominal)',
    ...
    const totalBars = granBarsG
        ...
        a.target = '_blank';
        a.href = `/sites/${(_a = d.id) !== null && _a !== void 0 ? _a : '12345'}`;
        a.click();
    });
...
//# sourceMappingURL=siteEnergyUsage.js.map