import { timeFormat, select, max, scaleLinear, scaleTime, extent, axisBottom, axisRight, timeParse, timeDay, selectAll, } from 'd3';
const fakeData = {
    data: [
        {
            baseline: 95,
...
export const makeGreenEngagement = (svgId, responseData) => {
    ...
    const data = fakeData.data.map((d, i) => {
        ...
        return Object.assign({ index: i, timeset, total: Math.random() * 100, dateNum: Date.parse(new Date(timeset)) }, d);
    });
    const dataXrange = extent(data, function (d) {
        return d.timeset;
    });
    const yMax = Math.ceil((_a = max(data, (d) => d.baseline)) !== null && _a !== void 0 ? _a : 0);
    const padY = yMax * 0.3;
    const aggregateYRange = [0, yMax + padY];
    x = scaleTime()
        .domain(dataXrange)
        .range([0, width - margin.left - margin.right - PAD - PAD]);
    y = scaleLinear().domain(aggregateYRange).range([0, -height]);
    const xAxis = (g, x) => {
        g.attr('transform', `translate(${PAD + margin.left - 6}, ${height + margin.top + 10})`).call(axisBottom(x)
            .scale(x)
            .tickSize(5)
            .ticks(10)
            .tickFormat((d) => {
    ...
    const hundredBars = granBarsG
        .selectAll('hundred-bar')
        .data(data)
        .enter()
        .append('rect')
        ...
        .attr('y', y(yMax) + height - margin.bottom - margin.top - BAR_Y_OFFSET)
        .attr('x', (d) => {
    ...
    const greenNotCaptured = granBarsG
        .selectAll('granular-rect')
        .data(data)
        .enter()
        .append('rect')
        .attr('fill', 'url(#hash4_4)');
    const totalText = granBarsG
        .selectAll('texts')
        .data(data)
        .enter()
        .append('text')
        ...
        .attr('id', (d) => `text-total--${d.index}`)
        .attr('class', `text-total--${svgId}`)
        .attr('fill', 'white')
        .attr('height', (d) => {
        return -y(d.baseline);
    })
    ...
    const text = granBarsG
        ...
        .attr('id', (d) => `text-percent--${d.index}`)
        .attr('class', `text-percent--${svgId}`)
        .attr('fill', 'white')
        .attr('height', (d) => {
        return -y(d.baseline);
    })
        .attr('y', (d) => y(d.baseline) + height - margin.bottom - BAR_Y_OFFSET - 7)
        .attr('x', (d) => {
        let len = (x(timeDay.offset(d.timeset)) - x(d.timeset)) * (BAR_WIDTH / 100);
        len = x(d.timeset) - len / 2;
        return len + PAD * 2 - 2;
    ...
    greenNotCaptured
        ...
        .attr('y', (d) => y(d.baseline) + height - margin.bottom - BAR_Y_OFFSET)
        .attr('width', (d) => {
    ...
    const compareBar = granBarsG
        ...
        .attr('y', (d) => height - margin.top + y(d.usage) - BAR_Y_OFFSET)
        .attr('width', (d) => {
        return (x(timeDay.offset(d.timeset)) - x(d.timeset)) * (BAR_WIDTH / 100);
    })
        .on('mouseover', function (event, d) {
...
//# sourceMappingURL=greenEngagementBar.js.map