...
import { axisLeft, axisTop, descending, max, pointer, scaleBand, scaleLinear, select, selectAll, } from 'd3';
...
export function makeBreakdownChart(svgId, initData) {
    var _a, _b;
    const THRESHOLD = (_a = initData.threshold) !== null && _a !== void 0 ? _a : 80, ABOVE_THRESHOLD_COLOR = 'var(--zss-warning)', BELOW_THRESHOLD_COLOR = 'var(--zss-nominal)';
    ...
    chartG
        ...
        .attr('ry', 8);
    xScale = scaleLinear()
        .domain([0, (_b = max(groupData, (d) => Number(d.KwH) + 50)) !== null && _b !== void 0 ? _b : 1500])
        .range([0, width])
        .nice();
    yScale = scaleBand()
        .domain(groupData.map((d) => d.title))
    ...
    barChildren = chartG
        ...
        .enter()
        .append('rect')
        .attr('class', (d) => `child-${d.parentId} chart-child`)
        .attr('fill', (d) => xScale(d.KwH) > xScale(THRESHOLD)
        ...
        .attr('y', (d) => { var _a; return (_a = yScale(d.title) + 3) !== null && _a !== void 0 ? _a : 0; })
        .attr('height', yScale.bandwidth())
        .attr('data-id', (d) => d.ChildName)
        .attr('data-parentId', (d) => d.title)
    ...
    chartG
        ...
        divTooltip
            ...
            `)
            .style('left', `${pointer(e)[0] - 10}px`)
            .style('top', e.pageY + 'px');
        selectAll(`.child-${d.parentId}`)
            .transition()
            .duration(50)
            .attr('opacity', '1')
            .attr('cursor', 'pointer');
...
//# sourceMappingURL=energySiteBreakdownChart.js.map