import { timeFormat, select, max, scaleLinear, scaleTime, extent, axisBottom, axisRight, axisTop, timeParse, utcFormat, sum, zoom, rollups, pointer, } from 'd3';
import { getGranular, getXMonthTicks, getXTicks, getXWidth, } from '../../utils/chartUtils';
...
export const generateEnergySummaryChart = (svgId, responseData, options) => {
    ...
    const data = jsonData.map((d, i) => {
        ...
        return Object.assign({ index: i, timeset: parseTime(d.time), dayOfYear: parseDayOfYear(timeset), dateNum: Date.parse(new Date(timeset)) }, d);
    });
    ...
    const initYMax = Math.ceil((_a = max(aggregateData, (d) => d.usage)) !== null && _a !== void 0 ? _a : 0);
    const { threshold } = responseData;
    THRESH_DAILY = threshold.daily;
    THRESH_HOURLY = threshold.hourly;
    const aggregateYRange = [0, (_b = initYMax + initYMax / 4) !== null && _b !== void 0 ? _b : 0];
    THRESH_SLIDER.select(`input#${svgId}--slider-input`).remove();
    var INPUT = THRESH_SLIDER.append('input')
        .attr('id', `${svgId}--slider-input`)
        .attr('class', 'mud-slider-input');
    INPUT.attr('type', 'range')
        .attr('min', 0)
        .attr('max', initYMax)
        .attr('value', THRESH_DAILY);
    ...
    const yAxis = (g, y) => {
        g.attr('transform', `translate(${margin.left}, ${-margin.top + height})`).call(axisRight(y)
            .scale(y)
            .tickSize(-width + 70)
            .ticks(5));
    ...
    const granBars = granBarsG
        ...
        toolTipDateDiv.text(() => { var _a; return hourFormat((_a = d.timeset) !== null && _a !== void 0 ? _a : new Date()); });
    ...
    function zoomed(event) {
        ...
        y.domain(isGranular
            ? [0, ((_a = granularYRange[1]) !== null && _a !== void 0 ? _a : 500) + ((_b = granularYRange[1]) !== null && _b !== void 0 ? _b : 6 / 6)]
            : aggregateYRange);
    ...
    SVG.call(zoomHandler)
        .transition()
        .delay(500)
        .call(zoomHandler.scaleTo, 0);
    const THRESH_GROUP = CHART_G.append('g')
        .attr('id', `${svgId}--threshold-group`)
        .attr('transform', `translate(0 , ${y(THRESH_DAILY) + height - margin.bottom})`);
    const THRESH_LINE = THRESH_GROUP.append('rect')
        .attr('width', width)
        .attr('id', `${svgId}--threshold-line`)
        .attr('fill', 'var(--zss-chart-axis-line)')
        .attr('height', '1')
        .attr('y', -2)
        .attr('x', 0);
    const THRESH_BG = THRESH_GROUP.append('rect')
        .attr('height', '30px')
        .attr('width', '180px')
        .attr('y', -45)
        .attr('x', width / 2 - 90)
        ...
        .attr('opacity', 0.7)
        .attr('ry', 5);
    THRESH_TEXT = THRESH_GROUP.append('text')
        .attr('id', `${svgId}--threshold-text`)
        .attr('fill', 'white')
        .attr('x', width / 2 + 3)
        .attr('y', -23)
    ...
    bars
        ...
        toolTipDateDiv.text(() => {
            var _a;
            const f = timeFormat('%m/%d');
            return f((_a = d.timeset) !== null && _a !== void 0 ? _a : new Date());
        });
        toolTipUsageDiv.text(`${d.usage} kwh`);
    })
        .on('mouseout', function (event, d) {
    ...
    bars.on('click', function (event, d) {
        SVG.call(zoomHandler)
            .transition()
            .duration(220)
            .call(zoomHandler.scaleTo, 14)
            .transition()
            .duration(220)
            .call(zoomHandler.translateTo, x(d.timeset), 0);
    });
};
...
//# sourceMappingURL=energySummaryChart.js.map