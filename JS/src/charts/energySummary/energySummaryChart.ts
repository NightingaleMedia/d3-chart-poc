...
export const generateEnergySummaryChart = (
  ...
  function updateBarThresh() {
    if (IS_GRANULAR) {
      ...
      THRESH_GROUP.attr(
        'transform',
        `translate(0,${y(THRESH_HOURLY) + height - margin.bottom})`
      );
      INPUT.attr('max', granularYRange[1]).attr('value', THRESH_HOURLY);
      THRESH_TEXT.text(`Threshold: ${THRESH_HOURLY}kwh`);
    } else {
      bars.attr('fill', (d) =>
        d.usage > THRESH_DAILY ? 'var(--zss-warning)' : 'var(--zss-nominal)'
      );

      THRESH_GROUP.attr(
        'transform',
        `translate(0 , ${y(THRESH_DAILY) + height - margin.bottom})`
      );
  ...
  function zoomed(
    ...
    y.domain(
      isGranular
        ? [0, (granularYRange[1] ?? 500) + (granularYRange[1] ?? 6 / 6)]
        : aggregateYRange
    );
  ...
  SVG.call(zoomHandler)
    .transition()
    .delay(500)
    .call(zoomHandler.scaleTo as any, 0);
