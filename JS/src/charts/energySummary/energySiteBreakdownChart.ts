...
export function makeBreakdownChart(
  ...
  const THRESHOLD = initData.threshold ?? 80,
  ...
  var margin = { top: 30, right: 0, bottom: 20, left: 0 },
    ...
    height = 90 * groupData.length - margin.top - margin.bottom + 90;
  //
  ...
  barChildren = chartG
    ...
    .attr('x', (d, i) => {
      ...
      let offset = 0;
      allChildren.forEach((ch, index: number) => {
        if (ch.index < d.index) {
          offset += ch.KwH + 5;
        }
      });
  ...
  chartG
    ...
    .on('mouseleave', function (e, d: EnergySiteDataItem) {
      divTooltip.style('opacity', '0');
      selectAll(`.child-${d.parentId}`).attr('opacity', '0.8');
    });

  svg.on('mouseleave', function (e, d: EnergySiteDataItem) {
    divTooltip.style('opacity', '0');
  });
}
