...
export function makeEngagementChart(svgId, data = { percent: 35 }) {
  ...
  var margin = { left: 0, top: 0, bottom: 0, right: 0 };
  var duration = 800;
  ...
  var defs = SVG.append('defs').attr('id', `chart-group--${svgId}`);
  var greenGradient = defs
  ...
  redGradient
    ...
    .attr('class', 'end')
    .attr('offset', '100%')
    .attr('stop-color', 'var(--zss-warning)')
    .attr('stop-opacity', 1);
  ...
  var lower_path = CHART_G.selectAll('path')
  ...
  var text = CHART_G.append('text')
    .attr('text-anchor', 'middle')
    .attr('dy', '.3em')
    .attr('fill', 'white')
    .attr('font-family', 'var(--zss-metric-font)')
    .attr('font-size', height * 0.15 + 'px');
