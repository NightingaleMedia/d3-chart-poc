...
export const makeGreenEngagement = (svgId, responseData) => {
  ...
  const xAxis = (g, x) => {
    g.attr(
      'transform',
      `translate(${PAD + margin.left - 6}, ${height + margin.top + 10})`
    ).call(
      axisBottom(x)
        .scale(x)
        .tickSize(5)
  ...
  const hundredBars = granBarsG
    ...
    .attr('x', (d) => {
      let len =
        (x(timeDay.offset(d.timeset)) - x(d.timeset)) * (BAR_WIDTH / 100);
      len = x(d.timeset) - len / 2;
      return len + PAD * 2;
    })
    .on('mouseover', function (event, d) {
      select(this).attr('opacity', 0.8);
      selectAll(`#text-percent--${d.index}, #text-total--${d.index}`).attr(
        'opacity',
        1
      );
    })
  ...
  greenNotCaptured
    ...
    .attr('width', (d) => {
      return (
        (x(timeDay.offset(d.timeset)) - x(d.timeset)) * (BAR_WIDTH / 100) - 0.5
      );
    })
    .on('mouseover', function (event, d) {
      select(this).attr('opacity', 0.8);
  ...
  SVG.on('mouseout', function (event, d) {
    selectAll(`.text-percent--${svgId}, .text-total--${svgId}`).attr(
      'opacity',
      0
    );
  });
